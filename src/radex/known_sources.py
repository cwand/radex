...
def sensitivity() -> tuple[float, float]:
    ...
    for src in ks_index:
        ...
        src_spec = radex.load_spectrum_from_file(config['calib']['calfiles'] + src + '.txt')

        # Calculate sensitivity (cps/Bq) from known source

        # Get rate in windows
        r_s = 0.0  # Net source rate
...
def write_calibration(spectrum: radex.Spectrum, activity: str, uncertainty_act: str, name: str) \
        -> str:
    # Read configuration
    config = configparser.ConfigParser()
    config.read('config.ini')
